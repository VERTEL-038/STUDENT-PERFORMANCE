...
def generate_report():
    ...
    for student in students:  # перебираем всех студентов
        avg_grade = calculate_average(student["grades"])  # считаем средний балл
        attendance_rate = get_attendance_rate(student["attendance"])  # считаем % посещаемости
    ...
    for student in students:
        ...
        print(f"Студент: {student['name']}")  # выводим имя
        print(f"   Средний балл: {avg_grade}")  # выводим средний балл
        print(f"   Посещаемость: {attendance_rate}%")  # выводим посещаемость

    # === УСПЕВАЕМОСТЬ ===
    print("─" * 80)
    print("УСПЕВАЕМОСТЬ\n")

    best_student = student_stats[0]  # сначала берём первого студента как лучшего
    worst_student = student_stats[0]  # и как худшего

    for s in student_stats:  # перебираем всех студентов
        if s["avg_grade"] > best_student["avg_grade"]:  # если нашли лучше
            best_student = s
        if s["avg_grade"] < worst_student["avg_grade"]:  # если нашли хуже
            worst_student = s
    ...
    best_class = classes[0]  # сначала берём первое занятие
    ...
    for i, class_name in enumerate(classes):
        ...
        # Обновляем самое посещаемое и самое непосещаемое занятие
        if percent > best_pct:
            best_pct = percent
